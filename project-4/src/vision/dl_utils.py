...
def compute_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    ...
    num_correct = torch.sum(torch.argmax(logits, dim = 1) == labels).cpu().item()
    batch_accuracy = num_correct / len(labels)
...
def compute_loss(
    ...
    if is_normalize:
        loss = model.loss_criterion(model_output, target_labels) / model_output.shape[0]
    else: 
