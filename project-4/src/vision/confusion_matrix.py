...
def generate_confusion_data(
    ...
    for idx, batch in enumerate(loader):
        input, target_labels = batch[0], batch[1]
        if cuda:
            input, target_labels = input.cuda(), target_labels.cuda()

        predictions = model.forward(input)
        predicted_labels = torch.argmax(predictions, dim=1)
        logits.extend(list(predicted_labels))
        targets.extend(list(target_labels))
...
def generate_confusion_matrix(
    ...
    for target, prediction in zip(targets, preds):
        ...
        confusion_matrix[int(target), int(prediction)] = confusion_matrix[int(target), int(prediction)] + 1
...
def generate_accuracy_data(
    ...
    for idx, batch in enumerate(loader.dataset):
