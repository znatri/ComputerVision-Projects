...
def compute_feature_distances(
    ...
    dists = np.zeros(shape=(features1.shape[0], features2.shape[0]))
    for idx in range(0, features2.shape[0]):
        dists[:, idx] = np.linalg.norm(features1 - features2[idx,:], axis=1)
...
def match_features_ratio_test(
    ...
    matches = np.empty((0,2)).astype(int)
    computed_dists = compute_feature_distances(features1, features2)

    for idx in range(0, computed_dists.shape[0]):

        sorted = np.argsort(computed_dists[idx, :])
        nndr = computed_dists[idx, sorted[0]] / computed_dists[idx, sorted[1]]

        if  0.89 > nndr > 0.4:
            matches = np.append(matches, np.reshape(np.array([idx, sorted[0]]), (1,2)), axis=0)
            confidences = np.append(confidences, np.reshape(np.array([nndr]), (1,1)))
