...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    img_data = np.reshape(image_bw, newshape=(1, 1, image_bw.shape[0], image_bw.shape[1]))
    img_data =  np.tile(img_data, (1, 1, 1, 1))
    img_data = torch.from_numpy(img_data).float()

    x_kernel = np.reshape(SOBEL_X_KERNEL, newshape=(1, 1, 3, 3))
    x_kernel = np.tile(x_kernel, (1, 1, 1, 1))
    x_kernel = torch.from_numpy(x_kernel).float()

    y_kernel = np.reshape(SOBEL_Y_KERNEL, newshape=(1, 1, 3, 3))
    y_kernel = np.tile(y_kernel, (1, 1, 1, 1))
    y_kernel = torch.from_numpy(y_kernel).float()

    x = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, stride=1, padding=(1, 1), bias=False)
    x.weight = nn.Parameter(x_kernel, requires_grad=False)
    Ix = x(img_data).numpy().squeeze()

    y = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, stride=1, padding=(1, 1), bias=False)
    y.weight = nn.Parameter(y_kernel, requires_grad=False)
    Iy = y(img_data).numpy().squeeze()

    Ix = np.reshape(Ix, newshape=(image_bw.shape[0], image_bw.shape[1]))
    Iy = np.reshape(Iy, newshape=(image_bw.shape[0], image_bw.shape[1]))
...
def compute_harris_response_map(
    ...
    detM = np.subtract(np.multiply(S_xx, S_yy), np.multiply(S_xy, S_xy))
    traceM = np.trace(M)
    R = detM - np.multiply(alpha, np.power(traceM, 2))
...
def nms_maxpool_pytorch(
    ...
    # Reshape R
    R = np.reshape(R, newshape=(1, 1, R.shape[0], R.shape[1]))
    R = torch.from_numpy(R).float()  

    # Set threshold
    median = torch.median(R)
    threshold = nn.Threshold(threshold=median, value=0)
    R = threshold(R)  

    # Pool over R to get local maxima
    output, indices = torch.nn.MaxPool2d(kernel_size=ksize, stride=1, return_indices=True)(R)
    unpool = torch.nn.MaxUnpool2d(kernel_size=ksize, stride=1)(output, indices)

    # Binarize all values > 0 to 1
    unpool[unpool!=0] = 1
    unpool = unpool.squeeze()

    # Store maximas in 1d arrays
    maximas = torch.nonzero(unpool)
    x = maximas[:,1]
    y = maximas[:,0]
    ...
    val, idx = c .sort(dim=0, descending=True)
    c  = (val[0:k]).numpy()
    idx = idx[0:k]

    x = x.index_select(0, idx).numpy()
    y = y.index_select(0, idx).numpy()
...
def get_harris_interest_points(
    ...
    R = compute_harris_response_map(image_bw=image_bw, ksize=7)
