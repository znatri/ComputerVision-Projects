...
def get_magnitudes_and_orientations(
    ...
    magnitudes = np.linalg.norm(np.array([Ix,Iy]), axis=0)
...
def get_gradient_histogram_vec_from_patch(
    ...
    wgh = np.zeros(shape=(0,0))

    for i in range(4): 
        for j in range(4):
            magnitude_grid = window_magnitudes[(4*i):(4*(i+1)),(4*j):(4*(j+1))]
            orientation_grid = window_orientations[(4*i):(4*(i+1)), (4*j):(4*(j+1))]
            histogram = np.histogram(orientation_grid, bins = 8, range = (-np.pi, np.pi), weights = magnitude_grid)
            wgh = np.append(wgh, histogram[0])
...
def get_feat_vec(
    ...
    magnitudes_grid = magnitudes[r - decr : r + incr, c - decr: c + incr]
    orientation_grid = orientations[r - decr : r + incr, c - decr: c + incr]

    wgh = get_gradient_histogram_vec_from_patch(magnitudes_grid, orientation_grid)
    fv = np.power(wgh / np.linalg.norm(wgh, 2) , 0.5)
...
def get_SIFT_descriptors(
    ...
    fvs = np.zeros(shape=(len(X), feature_width * 8))
    Ix,Iy = compute_image_gradients(image_bw)
    magnitudes, orientations = get_magnitudes_and_orientations(Ix, Iy)

    for idx in range(0, len(X)):
        fv = get_feat_vec(X[idx], Y[idx], magnitudes, orientations, feature_width)
        fvs[idx,:] = fv.squeeze()
...
def get_sift_features_vectorized(
    ...
    Ix,Iy = compute_image_gradients(image_bw)
    magnitudes, orientations = get_magnitudes_and_orientations(Ix, Iy)
    cv = nn.Conv2d(in_channels=1, out_channels=10, kernel_size=3, stride=1, padding=(1, 1), padding_mode=zeros)

    raise NotImplementedError('`get_ SIFT_features_vectorized` function in ' + '`part4_sift_descriptor.py` needs to be implemented')
