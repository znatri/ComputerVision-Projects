...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    mean = math.floor(ksize/2)

    x_val = np.ones(ksize)
    for i in range(0, ksize):
      x_val[i] = int(i)

    x_val = np.sqrt(7/44) * (1/sigma) * np.exp((-1/(2 * math.pow(sigma, 2))) * (x_val - mean)**2)
    kernel = np.outer(x_val, 1)
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    kernel1D = create_Gaussian_kernel_1D(int(cutoff_frequency * 4 + 1), cutoff_frequency)
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    padSize = [int(np.floor(x/2)) for x in filterSize]
    padded_image = np.lib.pad(image, ((padSize[0],padSize[0]),(padSize[1],padSize[1]),(0,0)), 'constant')

    if image.shape[2] == 1:
      for i in range(0, filtered_image.shape[0]):
        for j in range(0, filtered_image.shape[1]):
          filtered_image[i][j] = np.sum(np.multiply(padded_image[i:i+filterSize[0], j:j+filterSize[1]], filter))
    else:
      filter = filter.reshape((filterSize[0],filterSize[1],1))
      for i in range(0, filtered_image.shape[0]):
        for j in range(0, filtered_image.shape[1]):
          filtered_image[i:i+1, j:j+1, :]= np.sum(np.multiply(padded_image[i:i+filterSize[0], j:j+filterSize[1],:], filter), axis=(0,1))
